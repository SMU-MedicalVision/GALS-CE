...
from Nii_utils import Save_Parameter, setup_seed, NiiDataWrite, NiiDataRead
from data.dataset_gen import DatasetFromFolder, test_pred, inverser_norm_ct, normalization_ct
...
def pretrain(opt):
    ...
    train_writer = SummaryWriter(join(opt.checkpoints_dir, 'log/train'))
    val_writer = SummaryWriter(join(opt.checkpoints_dir, 'log/val'), flush_secs=4)
    ...
    for epoch in range(opt.epoch_count, opt.max_epochs + 1):
        ...
        if epoch % 1 == 0:
            ...
            if epoch_val_MAE_A2B_liver < best_MAE_A2B_val_liver:
                try:
                    os.remove(join(opt.model_results, f'best_MAE_val_liver_epoch{best_val_epoch_liver}.pth'))
                except:
                    pass
            ...
            if epoch_val_MAE_A2B_all < best_MAE_A2B_val_all:
                try:
                    os.remove(join(opt.model_results, f'best_MAE_val_all_epoch{best_val_epoch_all}.pth'))
                except:
                    pass
        ...
        try:
            os.remove(join(opt.model_results, f'latest_epoch{epoch-1}.pth'))
        except:
            pass
    ...
    train_writer.close()
    val_writer.close()
    return join(opt.model_results, f'best_MAE_val_liver_epoch{best_val_epoch_liver}.pth')
...
def main(opt):
    ...
    for epoch in range(opt.epoch_count, opt.max_epochs + 1):  # outer loop for different epochs; we save the model by <epoch_count>, <epoch_count>+<save_latest_freq>
        ...
        if epoch_val_MAE_mask < best_MAE_val_mask:
            try:
                os.remove(join(opt.model_results, f'best_MAE_mask_val_epoch{best_val_epoch_mask}.pth'))
            except:
                pass
        ...
        if epoch_val_MAE_all < best_MAE_val_all:
            try:
                os.remove(join(opt.model_results, f'best_MAE_all_val_epoch{best_val_epoch_all}.pth'))
            except:
                pass
    ...
    train_writer.close()
    val_writer.close()
    return model, join(opt.model_results, f'best_MAE_mask_val_epoch{best_val_epoch_mask}.pth')


def pred(opt, model_paths_dict, model, dataset='Inference', override=False):
    for phase in model_paths_dict.keys():
        print(model_paths_dict[phase])
        opt.model_results = os.path.dirname(model_paths_dict[phase])
        opt.load_name = os.path.basename(model_paths_dict[phase]).split('.pth')[0]
        load_networks(opt, model)
        model.eval()
        with torch.no_grad():
            wait_list = [ID for ID in os.listdir(join(opt.image_dir, dataset))]
            for i, ID in enumerate(wait_list):
                if os.path.exists(join(opt.image_dir, f'{dataset}_GALS-CE_syn', ID, f'{phase}.nii.gz')) and not override:
                    print(f'Skip {ID}, already exists')
                    continue
                NC, spacing, origin, direction = NiiDataRead(join(opt.image_dir, dataset, ID, 'NC.nii.gz'))
                NC_norm = normalization_ct(NC, opt.CT_min, opt.CT_max, opt.CT_mid1, opt.CT_mid2, opt.Norm_tr1, opt.Norm_tr2)
                b_mask, _, _, _ = NiiDataRead(join(opt.image_dir, dataset, ID, 'Body_mask.nii.gz'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--D_model', type=str, default='basicD', help='specify discriminator architecture [wave3DDiscriminator | n_layers | swinDiscriminator]. The basic model is a 70x70 PatchGAN. n_layers allows you to specify the layers in the discriminator')
    ...
    parser.add_argument('--train_model_path_AP', type=str, default='', help='model path for AP')
    parser.add_argument('--train_model_path_PVP', type=str, default='', help='model path for PVP')
    parser.add_argument('--train_model_path_DP', type=str, default='', help='model path for DP')
    ...
    if not opt_pre.save_dir and not opt_pre.inference_only:
        ...
        opt_pre.model_results = join(opt_pre.checkpoints_dir, 'model_results')
        opt_pre.file_name_txt = join(opt_pre.checkpoints_dir, 'train_message.txt')
        opt_pre.prediction_results = join(opt_pre.checkpoints_dir, 'prediction_results_')
    ...
    parser.add_argument('--D_model', type=str, default='trainD',
    ...
    parser.add_argument('--G_model', type=str, default='Pre_Swin_ADN',
    ...
    if not opt_train.save_dir and not opt_train.inference_only:
        ...
        for phase in ['AP', 'PVP', 'DP']:
            ...
            opt_train.checkpoints_name = '%s_%s_modal-%s' % (opt_train.G_model, opt_train.D_model,
                                                                 opt_train.target_modal)
            opt_train.checkpoints_dir = join('./main/trained_models/GALS-CE_gen/train', opt_train.checkpoints_name,
                                             save_name, current_time)
            opt_train.model_results = join(opt_train.checkpoints_dir, 'model_results')

            opt_train.val_results = join(opt_train.checkpoints_dir, 'val_results')
            opt_train.val_txt = join(opt_train.val_results, 'val_message.txt')
        ...
        if opt_train.train_model_path_AP:
            ...
            print(f'AP model path: {opt_train.train_model_path_AP}')
        if opt_train.train_model_path_PVP:
            model_paths_dict['PVP'] = opt_train.train_model_path_PVP
            print(f'PVP model path: {opt_train.train_model_path_PVP}')
        if opt_train.train_model_path_DP:
