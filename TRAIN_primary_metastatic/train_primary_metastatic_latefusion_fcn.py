...
from data.dataset_3D import Dataset_for_primary_metastatic_tumor
from Networks.ResNet_3D import ResNet18_3D_4stream_clinical_LSTM_latefusion_fcn
...
def main(args):
    ...
    train_data = Dataset_for_primary_metastatic_tumor(args.data_dir, args.train_split_path, args.metadata_path, augment=True)
    val_data = Dataset_for_primary_metastatic_tumor(args.data_dir, args.val_split_path, args.metadata_path, augment=False)
    test_data = Dataset_for_primary_metastatic_tumor(args.data_dir, args.test_split_path, args.metadata_path, augment=False)
    wtest_data = Dataset_for_primary_metastatic_tumor(args.data_dir, args.wtest_split_path, args.metadata_path, augment=False)
    ...
    val_dataloader = DataLoader(dataset=val_data, batch_size=args.bs, num_workers=min(8, args.bs), shuffle=False, drop_last=False)
    test_dataloader = DataLoader(dataset=test_data, batch_size=args.bs, num_workers=min(8, args.bs), shuffle=False, drop_last=False)
    wtest_dataloader = DataLoader(dataset=wtest_data, batch_size=args.bs, num_workers=min(8, args.bs), shuffle=False, drop_last=False)
    ...
    net = ResNet18_3D_4stream_clinical_LSTM_latefusion_fcn(in_channels=2, clinical_inchannels=3, n_classes=[3, 6], pretrained=False, no_cuda=False).cuda()
    ...
    for epoch in range(args.epoch):
        ...
        for i, DATA in enumerate(tqdm(train_dataloader)):
            ...
            # loss = CB_loss(labels, outputs, samples_per_cls=[train_data.num_0, train_data.num_1, train_data.num_2, train_data.num_3, train_data.num_4, train_data.num_5, train_data.num_6, train_data.num_7],
            #                no_of_classes=args.num_class, loss_type='focal', beta=0.999, gamma=2)
            loss = CB_loss(labels, outputs,
                           samples_per_cls=[1, 1, 1, 1, 1, 1, 1, 1],
                           no_of_classes=args.num_class, loss_type='focal', beta=0.999, gamma=2)
            if args.flood>0:
            ...
            for i, DATA in enumerate(tqdm(val_dataloader)):
                ...
                loss = CB_loss(labels, outputs,
                               samples_per_cls=[train_data.num_0, train_data.num_1, train_data.num_2, train_data.num_3,
                                                train_data.num_4, train_data.num_5, train_data.num_6, train_data.num_7],
                               no_of_classes=args.num_class, loss_type='focal', beta=0.999, gamma=2)
                outputs = torch.softmax(outputs, dim=1)
                predicted = torch.argmax(outputs, dim=1, keepdim=False).detach()
        ...
        with torch.no_grad():
            ...
            for i, DATA in enumerate(tqdm(wtest_dataloader)):
                Plain_imgs = DATA["Plain_img"].cuda().float()
                Arterial_imgs = DATA["Arterial_img"].cuda().float()
                Venous_imgs = DATA["Venous_img"].cuda().float()
                Delay_imgs = DATA["Delay_img"].cuda().float()
        ...
        try:
            ...
            try:
                print('wtest AUC error, use ovr instead.')
        ...
        if val_AUC > best_AUC_val:
            ...
            torch.save(net.state_dict(), os.path.join(args.checkpoints_dir, f'best_AUC_val_{epoch}.pth'))
        if val_ACC > best_ACC_val:
            best_ACC_val = val_ACC
            torch.save(net.state_dict(), os.path.join(args.checkpoints_dir, f'best_ACC_val_{epoch}.pth'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu', type=str, default='2', help='which gpu is used')
    ...
    # parser.add_argument('--data_dir', type=str, default='data/Liver_CE_classifiy_data_preprocessed_before_mask')
    parser.add_argument('--data_dir', type=str, default='data/Liver_CE_classifiy_data_preprocessed/')
    # parser.add_argument('--data_dir', type=str, default='data/weipeizhun_Synthesis_data_post_mask/')
    ...
    args.checkpoints_dir = 'trained_models/pretrain_freeze_primary_metastatic_tumor_3D_latefusion_FCN/bs{}_epoch{}_seed{}_{}'.format(args.bs, args.epoch, args.seed, current_time)
    args.L2 = 0.00005
