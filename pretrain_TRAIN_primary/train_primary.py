...
from data.dataset_3D import Dataset_for_primary_tumor
...
def main(args):
    ...
    train_data = Dataset_for_primary_tumor(args.data_dir, args.train_split_path, args.metadata_path, augment=True)
    val_data = Dataset_for_primary_tumor(args.data_dir, args.val_split_path, args.metadata_path, augment=False)
    test_data = Dataset_for_primary_tumor(args.data_dir, args.test_split_path, args.metadata_path, augment=False)
    ...
    print('train_lenth: %i  val_lenth: %i test_lenth: %i  num_0: %i  num_1: %i  num_2: %i' % (
        train_data.len, val_data.len, test_data.len, train_data.num_0, train_data.num_1, train_data.num_2))

    net = ResNet18_3D_4stream_clinical_LSTM(in_channels=2, clinical_inchannels=3, n_classes=args.num_class, pretrained=False, no_cuda=False).cuda()
    if args.pretrain:
    ...
    for epoch in range(args.epoch):
        ...
        for i, DATA in enumerate(train_dataloader):
            ...
            loss = CB_loss(labels, outputs, samples_per_cls=[train_data.num_0, train_data.num_1, train_data.num_2],
        ...
        with torch.no_grad():
            ...
            for i, DATA in enumerate(val_dataloader):
                ...
                loss = CB_loss(labels, outputs,
                               samples_per_cls=[train_data.num_0, train_data.num_1, train_data.num_2],
                               no_of_classes=args.num_class, loss_type='focal', beta=0.999, gamma=2)
                outputs = torch.softmax(outputs, dim=1)
                predicted = torch.argmax(outputs, dim=1, keepdim=False).detach()
        ...
        print(
            '[%d/%d] train_loss: %.3f train_AUC: %.3f val_AUC: %.3f test_AUC: %.3f train_ACC: %.3f val_ACC: %.3f test_ACC: %.3f' %
            (epoch, args.epoch, train_epoch_loss, train_AUC, val_AUC, test_AUC, train_ACC, val_ACC, test_ACC))
...
if __name__ == '__main__':
    ...
    args.checkpoints_dir = 'trained_models/primary_tumor_3D/bs{}_epoch{}_seed{}_{}'.format(args.bs, args.epoch, args.seed,
                                                                            current_time)
    args.L2 = 0.00005
