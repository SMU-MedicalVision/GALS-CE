...
# from Contrast_Methods.network_architecture.initialization import InitWeights_He
# from Contrast_Methods.network_architecture.neural_network import SegmentationNetwork
...
class ShareSepConv2d(nn.Module):
    def __init__(self, kernel_size):
        super(ShareSepConv2d, self).__init__()
...
class SmoothDilatedResidualBlock2d(nn.Module):
    def __init__(self, channel_num, dilation=1, group=1):
        super(SmoothDilatedResidualBlock2d, self).__init__()
        self.pre_conv1 = ShareSepConv2d(dilation*2-1)
        self.conv1 = nn.Conv2d(channel_num, channel_num, 3, 1, padding=dilation, dilation=dilation, groups=group, bias=False)
        self.norm1 = nn.InstanceNorm2d(channel_num, affine=True)
        self.pre_conv2 = ShareSepConv2d(dilation*2-1)
...
class WindowAttention1(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.qkv_dwconv1 = SmoothDilatedResidualBlock2d(num_heads, dilation=2, group=num_heads)
        self.qkv_dwconv2 = SmoothDilatedResidualBlock2d(num_heads, dilation=2, group=num_heads)
        self.qkv_dwconv3 = SmoothDilatedResidualBlock2d(num_heads, dilation=2, group=num_heads)
    ...
    def forward(self, x, mask=None):
        ...
        qkv = qkv.reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
        q = self.qkv_dwconv1(q)
        k = self.qkv_dwconv2(k)
        v = self.qkv_dwconv3(v)
...
class BasicLayer2(nn.Module):
    ...
    def __init__(self,
        ...
        self.window_size_total.append(self.window_size)
        self.window_size_total.append([self.window_size[0], self.window_size[1] // 2, self.window_size[2] * 4])
        self.window_size_total.append([self.window_size[0], self.window_size[1] * 4, self.window_size[2] // 2])
        ...
        self.shift_size_total.append(
            [self.window_size_total[2][0] // 2, self.window_size_total[2][1] // 2, self.window_size_total[2][2] // 2])
        ...
        self.block1 = nn.ModuleList([
            SwinTransformerBlock2(
                ...
                shift_size=[0, 0, 0] if (i % 2 == 0) else self.shift_size_total[0],
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
                drop=drop,
        ...
        self.block2 = nn.ModuleList([
            SwinTransformerBlock2(
                ...
                shift_size=[0, 0, 0] if (i % 2 == 0) else self.shift_size_total[1],
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
                drop=drop,
        ...
        self.block3 = nn.ModuleList([
            SwinTransformerBlock2(
                ...
                input_resolution=input_resolution,
                num_heads=num_heads // 4,
                window_size=self.window_size_total[2],
                shift_size=[0, 0, 0] if (i % 2 == 0) else self.shift_size_total[2],
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
                drop=drop,
    ...
    def forward(self, x, S, H, W):
        ...
        for idx in range(3):
            ...
            if idx == 0:
                block = self.block1
                x_this = x[:, :, 0:self.dim // 2]
            elif idx == 1:
                block = self.block2
                x_this = x[:, :, self.dim // 2:self.dim // 4 * 3]
            elif idx == 2:
                block = self.block3
                x_this = x[:, :, self.dim // 4 * 3:]
            ...
            for blk in block:
                blk.H, blk.W = H, W
                if self.use_checkpoint:
                    x_this = checkpoint.checkpoint(blk, x_this, attn_mask)
                else:
                    x_this = blk(x_this, attn_mask)
            x_total.append(x_this)

        x = torch.cat([x_total[0],x_total[1],x_total[2]],dim=2)
        x = self.dim_mix(x)
...
class BasicLayer3(nn.Module):
    ...
    def __init__(self,
        ...
        self.block3 = nn.ModuleList([
            SwinTransformerBlock3(
                dim=int(dim * self.dim_change[2]),
                input_resolution=input_resolution,
                num_heads=int(num_heads * self.dim_change[2]),
                window_size=self.window_size_total[2],
                shift_size=[0, 0, 0] if (i % 2 == 0) else self.shift_size_total[2],
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
    ...
    def forward(self, x, S, H, W):
        ...
        for idx in range(3):
            ...
            if idx ==0:
                ...
                block = self.block2
                x_this = x[:, :, int(self.dim * self.dim_change[0]):int(self.dim * self.dim_change[0]+self.dim * self.dim_change[1])]
            elif idx==2:
                block = self.block3
                x_this = x[:, :, int(self.dim * self.dim_change[0]+self.dim * self.dim_change[1]):]
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=4, embed_dim=96, norm_layer=None):
        ...
        self.proj1 = project(in_chans,embed_dim//4,[1,2,2],1,nn.GELU,nn.LayerNorm,False)
        self.proj2 = project(embed_dim//4, embed_dim // 2, [1, 2, 2], 1, nn.GELU, nn.LayerNorm, False)
        self.proj3 = project(embed_dim//2,embed_dim,[2,2,2],1,nn.GELU,nn.LayerNorm,True)
...
class project_patch(nn.Module):
    def __init__(self, in_dim, out_dim, kernel_size, stride, activate, norm, last=False):
        super().__init__()
        self.out_dim = out_dim
        self.conv1 = nn.Conv3d(in_dim, out_dim, kernel_size=kernel_size, stride=stride)
        self.conv2 = nn.Conv3d(out_dim, out_dim, kernel_size=1, stride=1)
        self.activate = activate()
        self.norm1 = norm(out_dim)
        self.last = last
        if not last:
